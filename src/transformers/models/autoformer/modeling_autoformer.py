...
class AutoformerAttention(nn.Module):
    ...
    def forward(
        ...
        self.outputs.append(query_states)
        ...
        self.outputs.append(key_states)
        self.outputs.append(value_states)
        ...
        self.outputs.append(query_states_fft)
        key_states_fft = torch.fft.rfft(key_states, n=tgt_len, dim=1)
        self.outputs.append(key_states_fft)
        attn_weights = query_states_fft * torch.conj(key_states_fft)
        self.outputs.append(attn_weights)
        ...
        self.outputs.append(autocorrelations)
        ...
        self.outputs.append(autocorrelations_mean_on_head_channel)
        if self.training:
        ...
        self.outputs.append(top_k_autocorrelations)
        ...
        #breakpoint()
        self.outputs.append(delays_agg)
        self.outputs.append(init_index)
        self.outputs.append(top_k_delays_index)
        #breakpoint()
        # breakpoint()
        for i in range(top_k):
            ...
            if not self.training:
                # breakpoint()
                tmp_delay = init_index + top_k_delays_index[:, i].view(-1, 1, 1, 1).repeat(
                    1, self.num_heads, tgt_len, channel
                ).view(-1, tgt_len, channel)
                # breakpoint()
                self.outputs.append(tmp_delay)
                value_states_roll_delay = torch.gather(tmp_values, dim=1, index=tmp_delay)
                self.outputs.append(value_states_roll_delay)
            else:
            ...
            self.outputs.append(top_k_autocorrelations_at_delay)
            delays_agg = delays_agg + value_states_roll_delay * top_k_autocorrelations_at_delay
            self.outputs.append(delays_agg)
